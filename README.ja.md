...
`amygdala` は差し替え可能な事実メモリ基盤(既定は
[mnemosyne](https://github.com/mnemosyne-oss/mnemosyne)、MIT)の上に乗せる、
**バックエンド非依存**の薄いレイヤです。以下を足します。
...
amygdala 本体に実行時のハード依存はありません(標準ライブラリのみ)。
`pip install amygdala` はバックエンド非依存のコアだけを入れ、記憶基盤は
extra で選びます:

```bash
pip install amygdala             # バックエンド非依存のコア(依存ゼロ)
pip install amygdala[mnemosyne]  # + 既定の mnemosyne バックエンド
```

既定バックエンド(`RealCore`)を使うなら `[mnemosyne]` を、別の記憶基盤を
使うなら `Core` アダプタを自作します(→「任意のバックエンドに載せる」)。
検索精度(vec + FTS5 ハイブリッド)は mnemosyne に任せ、最終ランクのみ
amygdala が決めます。
...
amygdala 本体に**実行時のハード依存はありません**。既定の
`pip install amygdala` はバックエンド非依存のコアのみを入れます。
`pip install amygdala[mnemosyne]` で既定バックエンドを追加でき、独自
バックエンドならどちらも不要です。
...
```bash
pip install -e ".[dev,mnemosyne]"  # mnemosyne は contract test 用
pytest
```
