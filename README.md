...
A版のコアには手を入れず、`pip install` の依存として使います。検索精度
（vec + FTS5 ハイブリッド）は A版に任せ、最終ランクのみ amygdala が決めます。

## 設計の要: 二段ランク

依存利用では A版のスコア式を改変できないため、A版に広めに候補を出させ、
感情強度・関係相手一致・STM境界除外を amygdala 側で適用して再ランクします。

```
A版 recall(query, top_k=24)  →  STM除外  →  感情/関係性で再ランク  →  上位 k
```

## write を遅くしない

感情推定（LLM/分類器）は write 経路から切り離し、背景ワーカで処理します。
A版の高速 write を維持し、未推定の間は neutral（無）既定で動作します。
...
# 体験を記録（A版 episodic + 背景で感情推定）
mid = router.remember("ユーザは昇進して喜んでいた", partner_id="user_42")

# 知識を記録（A版 temporal triple、感情なし）
