...
| 0.4 | 実 classifier のリファレンス実装例(examples/、依存には入れない) | 未着手 |
| 1.0 | 公開 API 確定(PUBLIC_API.md)+ 日英 README + PyPI 公開 | 未着手 |
...
- [ ] clean environment で `pip install .` が成功する。
- [ ] `from amygdala import MemoryRouter, Emotion` が成功する。
- [ ] 対応 mnemosyne バージョンを下限・上限付きで明記し、実 SDK との contract test が通る。
- [ ] 同じ本文で partner A / B の記憶を作り、A を指定した recall で A の候補が
      partner boost を受ける統合テストが通る。
- [ ] classifier 例外、DB 例外、shutdown 中の未処理ジョブで worker が無言停止しない。
- [ ] 同じ job を 2 回処理しても affinity / trust / mood が 2 回加算されない。
- [ ] RelationStore の同時更新テストで lost update が発生しない。
- [ ] 不正 ULID / 非 ULID / None の STM 境界が安全に処理される。
- [ ] ベース recall と再ランク後の評価データを保存し、重みによる明白な品質劣化がない。
- [ ] partner 単位の delete/export と、孤児 emotion レコードの清掃方針が文書化される。

### P1: 0.2 までに満たす

- [ ] queue depth / failure / fallback / latency の stats を取得できる。
- [ ] 永続キューを採用しない場合、プロセス異常終了時に感情推定が失われ得ることを
      README に明記する。
- [ ] salience / valence / relationship impact / response policy を API と文書で区別する。
- [ ] prompt injection を含む記憶本文が injection block の命令へ昇格しないテストを持つ。
