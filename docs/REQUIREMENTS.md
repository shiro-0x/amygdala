...
4. OCC/PAD マッピングの提供時期(相互運用の需要が出るまで保留)。
   補足(1.4): amygdala 本体はバックエンド非依存(`Core` Protocol の 3
   メソッドのみに依存)であることを明文化・例示(`examples/custom_backend.py`)。
   mnemosyne 以外(ベクトル DB 等)にも Core アダプタを書けば載る。
   **2.0 でハード依存を解除**: `pip install amygdala` は依存ゼロ、記憶基盤は
   `[mnemosyne]` extra または独自 Core アダプタで選ぶ(オーナー判断で実施)。
5. ~~hersona 側 skill(`/hersona`)から amygdala の気分ブロックを参照する UX~~
   → **決定(1.3)**: **トークンコストがかからない方向**を採用。連携ロジックは
   SKILL.md(毎ターン LLM コンテキストに載る)へは入れず、アプリ側コードの
   ヘルパー `compose_system_prompt`(LLM 非依存)+ オンデマンド読込の
   REFERENCE レシピで完結させる。`/hersona` skill の毎ターン token コストは
   増えない(増えるのは感情ペイロード = state_block そのものだけ)。
