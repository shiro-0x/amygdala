...
> **注 (バージョンリセット)**: 下記 0.1.0〜2.0.0 は初期構築セッション内の
> **開発履歴**。初回公開バージョンはオーナー判断で **0.0.1** にリセットした
> (中身は 2.0.0 の開発状態と機能的に同一 = バックエンド非依存・依存ゼロ)。
> 履歴は消さず保持する。0.x のため公開 API は未凍結(破壊的変更が minor で
> 起こりうる)。

## [0.0.1] - 2026-07-18

初回公開バージョン。機能は下記 2.0.0 の開発状態と同一:

- 喜怒哀楽+無の 5 値感情、関係性進行、現在の気分(二速力学)、二段ランク想起、
  STM 境界除外、プロンプト注入ブロック / JSON export(FR-6.5 出力規約込み)、
  hersona 並置ヘルパー `compose_system_prompt`
- **バックエンド非依存・実行時ハード依存ゼロ**(`Core` protocol の 3 メソッド。
  既定は `pip install amygdala[mnemosyne]`、任意バックエンドは
  `examples/custom_backend.py`)
- オプトイン: 感情相互作用 / milestone 自動検出 / recall 動的重み
- 110 テスト + CI(バックエンド非依存ジョブ含む)

---

以下は初期構築セッション内の開発履歴(公開前)。
